...
mod layer_1_1_nats_connection {
    ...
    async fn test_graph_event_publishing() {
        ...
        validator.create_test_stream("test-graph-events").await.unwrap();
        
        // Publish context graph events
        let events = vec![
            ("test-graph-events.node.added", r#"{"node_id": "n1", "context": "test"}"#),
            ("test-graph-events.edge.created", r#"{"source": "n1", "target": "n2"}"#),
            ("test-graph-events.context.updated", r#"{"context_id": "ctx1", "changes": []}"#),
        ];
        ...
        let valid = validator.validate_event_sequence("test-graph-events", 3).await.unwrap();
...
mod layer_1_2_event_store {
    ...
    async fn test_context_graph_replay() {
        ...
        let mut stream = validator.jetstream.get_stream("graph-replay").await.unwrap();
...
mod layer_1_3_graph_operations {
    ...
    async fn test_distributed_graph_sync() {
        ...
        let operations = vec![
            ("graph-sync.node.add", r#"{"source": "client-1", "node_id": "n1"}"#),
            ("graph-sync.node.add", r#"{"source": "client-2", "node_id": "n2"}"#),
            ("graph-sync.edge.add", r#"{"source": "client-1", "from": "n1", "to": "n2"}"#),
            ("graph-sync.context.merge", r#"{"source": "client-2", "contexts": ["ctx1", "ctx2"]}"#),
        ];
        
        for (subject, payload) in operations {
            validator.client.publish(subject, payload.into()).await.unwrap();
        }
        
        tokio::time::sleep(Duration::from_millis(100)).await;
        let valid = validator.validate_event_sequence("graph-sync", 4).await.unwrap();
    ...
    async fn test_graph_composition_events() {
        let validator = EventStreamValidator::new().await.unwrap();
        validator.create_test_stream("graph-composition").await.unwrap();
        ...
        for (i, event) in composition_events.iter().enumerate() {
            ...
            validator.client.publish(subject, event.as_bytes().to_vec().into()).await.unwrap();
        }
        
        // Verify composition event ordering
        let mut stream = validator.jetstream.get_stream("graph-composition").await.unwrap();
    ...
    async fn test_conceptual_space_mapping() {
        let validator = EventStreamValidator::new().await.unwrap();
        validator.create_test_stream("conceptual-mapping").await.unwrap();
        ...
        let mut stream = validator.jetstream.get_stream("conceptual-mapping").await.unwrap();
        let info = stream.info().await.unwrap();
    ...
    async fn test_graph_query_cache_invalidation() {
        ...
        let mutation_events = vec![
            ("graph-cache.mutation.node_added", r#"{"graph_id": "g1", "node_id": "n1"}"#),
            ("graph-cache.invalidate.query", r#"{"graph_id": "g1", "query_types": ["neighbors", "paths"]}"#),
            ("graph-cache.mutation.edge_removed", r#"{"graph_id": "g1", "edge_id": "e1"}"#),
            ("graph-cache.invalidate.all", r#"{"graph_id": "g1", "reason": "structural_change"}"#),
        ];
        ...
        tokio::time::sleep(Duration::from_millis(100)).await;
        let valid = validator.validate_event_sequence("graph-cache", 4).await.unwrap();
