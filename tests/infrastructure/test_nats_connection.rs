...
async fn test_nats_connection_and_event_flow() {
    ...
    while let Some(msg) = messages.next().await {
        match msg {
            Ok(msg) => {
                let event: TestEvent =
                    serde_json::from_slice(&msg.payload).expect("Failed to deserialize event");
    ...
    assert_eq!(
        captured_events, EXPECTED_SEQUENCE,
...
async fn test_concurrent_event_publishing() {
    ...
    assert_eq!(
        info.state.messages, total_published as u64,
