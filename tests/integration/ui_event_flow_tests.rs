...
use bevy::prelude::*;
use ia::{
    simple_agent::{AgentQuestionEvent, AgentResponseEvent, AgentErrorEvent, SimpleAgentPlugin},
...
fn test_ui_to_agent_event_flow() {
    ...
    app.add_systems(Update, move |mut events: EventReader<AgentQuestionEvent>| {
...
fn test_event_ordering() {
    ...
    assert_eq!(tracker.questions, questions, "Events should be processed in order");
}
...
fn test_response_event_payload() {
    ...
    assert_eq!(tracker.responses[0], expected_response, "Response payload should match");
}
...
fn test_complete_qa_cycle() {
    ...
    assert!(tracker.last_question.is_some(), "Should have captured question");
    assert!(tracker.last_response.is_some(), "Should have captured response");
}
