...
fn test_ui_event_flow() {
    ...
    assert_eq!(
        tracker.questions.len(),
        1,
        "Should have captured one question"
    );
    assert_eq!(
        tracker.questions[0], "What is CIM?",
        "Question content should match"
    );
    ...
    let tracker = app.world().resource::<EventTracker>();
    assert_eq!(
        tracker.responses.len(),
        1,
        "Should have captured one response"
    );
    assert_eq!(
        tracker.responses[0], "CIM is a Composable Information Machine",
...
fn test_event_ordering() {
    ...
    app.add_systems(
        Update,
        |mut events: EventReader<AgentQuestionEvent>, mut tracker: ResMut<OrderTracker>| {
            for event in events.read() {
    ...
    let questions = vec!["First question", "Second question", "Third question"];
    ...
    assert_eq!(
        tracker.order[1], "Second question",
...
fn test_complete_ui_flow() {
    ...
    app.add_systems(
        Update,
        |mut events: EventReader<AgentResponseEvent>, mut ui_state: ResMut<UiState>| {
            for event in events.read() {
    ...
    assert_eq!(
        ...
        2,
        "Should have question and response"
    );
    assert_eq!(
        ui_state.messages[0].0, "What are the 8 CIM domains?",
        "First message should be question"
    );
    assert!(ui_state.messages[0].1, "First message should be from user");
    assert_eq!(
        ui_state.messages[1].0, "Response to: What are the 8 CIM domains?",
        "Second message should be response"
    );
    assert!(
        !ui_state.messages[1].1,
        "Second message should not be from user"
    );
