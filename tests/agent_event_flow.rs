...
use ia::simple_agent::{AgentQuestionEvent, AgentResponseEvent, AgentErrorEvent};
use ia::plugins::{AgentUiPlugin, AgentIntegrationPlugin};
...
fn test_quick_action_events() {
    ...
    app.world_mut().resource_mut::<ButtonInput<KeyCode>>().release(KeyCode::F2);
    ...
    app.world_mut().resource_mut::<ButtonInput<KeyCode>>().release(KeyCode::F3);
    app.world_mut().resource_mut::<ButtonInput<KeyCode>>().press(KeyCode::F4);
    app.update();
