...
use anyhow::Result;
use chrono::Utc;
use cim_domain_agent::{Agent, AgentType, Participant as AgentParticipant};
use cim_domain_dialog::{
    AddContextVariable, AddTurn, ContextScope, ContextVariable, Dialog, DialogStatus, DialogType,
    Message, MessageIntent, Participant, ParticipantRole, ParticipantType, StartDialog,
    SwitchContext, Topic, Turn, TurnType,
};
...
impl DialogAgent {
    ...
    async fn process_message(
        ...
        if self
            .has_topic_changed(message, dialog.current_topic())
            .await?
        {
    ...
    async fn extract_context_variables(
        &self,
        message: &str,
    ...
    async fn generate_contextual_response(&self, dialog: &Dialog, message: &str) -> Result<String> {
        ...
        Ok(format!(
            ...
            dialog
                .current_topic()
                .map(|t| &t.name)
                .unwrap_or(&"general topics".to_string()),
            dialog.turns().len(),
            message,
            context_summary
        ))
    ...
    async fn create_conversation_snippet(
        &self,
        dialog: &Dialog,
        last_n_turns: usize,
    ) -> Result<String> {
...
async fn main() -> Result<()> {
    ...
    // Start conversation
    let dialog_id = agent
        .start_conversation(user_id, "What is event sourcing and how does it work?")
        .await?;
    ...
    let response1 = agent
        .process_message(dialog_id, user_id, "Can you explain CQRS as well?")
        .await?;
