...
use bevy::prelude::*;
use ia::simple_agent::{AgentQuestionEvent, AgentResponseEvent, AgentErrorEvent, SimpleAgentPlugin};
...
fn log_response_events(
    ...
    for event in events.read() {
        ...
        log.responses_received.push((event.response.clone(), timestamp));
    }
}
