...
use ia::presentation::systems::{forward_import_requests, process_graph_import_requests, forward_import_results};
...
fn log_commands(mut events: EventReader<CommandEvent>) {
    for event in events.read() {
        println!("1. LOG_COMMANDS: Received command: {:?}", event.command.command_type());
    }
}
...
fn process_test_commands(
    ...
    for cmd in commands.read() {
        println!("2. PROCESS_COMMANDS: Processing command: {:?}", cmd.command.command_type());
...
fn log_event_notifications(mut events: EventReader<EventNotification>) {
    for event in events.read() {
        println!("3. LOG_EVENT_NOTIFICATIONS: Received event: {:?}", event.event.event_type());
    }
}

fn log_import_requests(mut events: EventReader<ImportRequestEvent>) {
    for event in events.read() {
        println!("5. LOG_IMPORT_REQUESTS: Received import request: {:?}", event.event.event_type());
    }
}

fn log_import_results(mut events: EventReader<ImportResultEvent>) {
    for event in events.read() {
        println!("7. LOG_IMPORT_RESULTS: Received import result: {:?}", event.event.event_type());
    }
}

fn log_final_events(mut events: EventReader<EventNotification>) {
    for event in events.read() {
        println!("9. LOG_FINAL_EVENTS: Final event: {:?}", event.event.event_type());
    }
}
