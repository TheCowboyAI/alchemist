...
use crate::domain::value_objects::{WorkflowId, GraphId};
use crate::infrastructure::nats::{NatsClient, WorkflowSubjectMapper, WorkflowEventType};
...
where
    S: WorkflowState + Clone + Send + Sync + 'static + serde::Serialize + for<'de> serde::Deserialize<'de>,
    I: TransitionInput + Clone + Send + Sync + 'static + serde::Serialize + for<'de> serde::Deserialize<'de>,
    O: TransitionOutput + Clone + Send + Sync + 'static + serde::Serialize + for<'de> serde::Deserialize<'de>,
{
    ...
    pub async fn start_workflow(
        ...
        let definition = definitions.get(&definition_id)
            .ok_or_else(|| DomainError::NotFound(format!("Workflow definition not found: {}", definition_id)))?;

                // Get initial state from definition
        let initial_state = definition.get_initial_state()
            .ok_or_else(|| DomainError::Validation("No initial state in workflow definition".to_string()))?
            .clone();
    ...
    pub async fn execute_transition(
        ...
        let workflow = workflows.get_mut(workflow_id)
            .ok_or_else(|| DomainError::NotFound(format!("Workflow instance not found: {}", workflow_id)))?;

        // Get the definition
        let definitions = self.definitions.read().await;
        let definition = definitions.get(&workflow.definition_id)
        ...
        if transitions.is_empty() {
            return Err(DomainError::Validation("No applicable transition found".to_string()));
        }
        ...
        if !transition.guard(&workflow.context) {
            return Err(DomainError::Validation("Transition guard failed".to_string()));
        }

        // Execute the transition (side effects only)
        transition.execute(&mut workflow.context)
