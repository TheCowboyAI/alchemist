...
pub fn process_commands(
    ...
    for command_event in commands.read() {
        ...
        match &command_event.command {
            ...
            Command::Workflow(_) => {
                warn!("Workflow commands should be handled by async handler");
                // Workflow commands are handled by the async WorkflowCommandHandler
            }
            Command::ConceptualSpace(_) => {
                warn!("ConceptualSpace commands should be handled by async handler");
                // ConceptualSpace commands are handled by the async ConceptualSpaceCommandHandler
            }
        }
    }
}
