...
use crate::domain::{
    commands::{Command, EdgeCommand, ImportSource, NodeCommand},
    events::{DomainEvent, GraphEvent},
    value_objects::{EdgeId, EdgeRelationship, NodeContent, NodeId, NodeType, RelationshipType},
};
use crate::presentation::components::{BoundaryType, SubgraphRegion};
...
pub fn process_graph_import_requests(
    ...
    for request in import_requests.read() {
        ...
        {
            eprintln!(
                "process_graph_import_requests: Processing import request for graph {:?}",
                graph_id
            );
            ...
            match content_result {
                Ok(content) => {
                    eprintln!(
                        "process_graph_import_requests: Content loaded, format requested: {}",
                        format
                    );
                    eprintln!(
                        "process_graph_import_requests: First 100 chars of content: {}",
                    ...
                    eprintln!(
                        "process_graph_import_requests: Using import format: {:?}",
                        import_format
                    );
                    ...
                    match import_service.import_from_content(
                        ...
                        Ok(imported_graph) => {
                            eprintln!(
                                "process_graph_import_requests: Successfully imported {} nodes and {} edges",
                                imported_graph.nodes.len(),
                                imported_graph.edges.len()
                            );
                            info!(
                                "Successfully imported {} nodes and {} edges",
                                imported_graph.nodes.len(),
                                imported_graph.edges.len()
                            );
                            ...
                            if has_subgraphs {
                                eprintln!(
                                    "process_graph_import_requests: Import contains subgraphs"
                                );
                                    ...
                                    if let Some(subgraph_array) = subgraphs.as_array() {
                                        for subgraph_info in subgraph_array {
                                            if let (Some(id), Some(name), Some(node_ids)) = (
                                                ...
                                                eprintln!(
                                                    "Created subgraph region: {} with {} nodes",
                                                    name,
                                                    node_set.len()
                                                );
                                            }
                            ...
                            if let Some(offset) = &options.position_offset {
                                eprintln!(
                                    "process_graph_import_requests: Applying position offset: {:?}",
                                    offset
                                );
                            ...
                            for node in &nodes {
                                eprintln!(
                                    "process_graph_import_requests: Creating node {} at position {:?}",
                                    node.label, node.position
                                );
                                ...
                                metadata.insert(
                                    "label".to_string(),
                                    serde_json::json!(node.label.clone()),
                                );
                                metadata.insert(
                                    "node_type".to_string(),
                                ...
                                if let Some(subgraph_name) =
                                    node.properties.get("subgraph").and_then(|v| v.as_str())
                                {
                                    metadata.insert(
                                        "subgraph".to_string(),
                                        serde_json::json!(subgraph_name),
                                    );
                                }
                                ...
                                let node_command = Command::Node(NodeCommand::AddNode {
                                    ...
                                    content: NodeContent {
                                        ...
                                        node_type: match node.node_type.as_str() {
                                            ...
                                            "Repository" => {
                                                NodeType::Custom("Repository".to_string())
                                            }
                            ...
                            for edge in &imported_graph.edges {
                                eprintln!(
                                    "process_graph_import_requests: Creating edge from {} to {}",
                                    ...
                                    let edge_command = Command::Edge(EdgeCommand::ConnectEdge {
                                        ...
                                        relationship: EdgeRelationship {
                                            relationship_type: match edge.edge_type.as_str() {
                                                ...
                                                _ => {
                                                    RelationshipType::Custom(edge.edge_type.clone())
                                                }
                                            },
...
mod tests {
    ...
    fn test_system_parameter_conflict() {
        ...
        assert!(
            true,
            "Conflict was resolved by introducing ImportRequestEvent"
        );
    }
