...
impl Position3D {
    /// Create a new Position3D with validation
    ...
    /// Create a Position3D without validation (for deserialization)
...
impl Default for Position3D {
    fn default() -> Self {
        // Safe to use new_unchecked here as 0.0 is always finite
...
impl TryFrom<Vec3> for Position3D {
...
///
/// In category theory and graph theory:
/// - Nodes represent OBJECTS (things that exist)
/// - Edges represent MORPHISMS (relationships, processes, constraints between objects)
///
/// All nodes must have a type. Use `Unidentified` if the type is not yet known.
...
pub enum NodeType {
    ...
    Custom(String), // Domain-specific type
}

/// Edge representing a morphism between nodes
///
/// Edges are the morphisms (arrows) in our category-theoretic graph.
/// They represent relationships, processes, constraints, or any other
/// connection between objects (nodes).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
/// Types of morphisms (relationships) between objects
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl fmt::Display for RelationshipType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            // Process
            RelationshipType::FlowsTo => write!(f, "FlowsTo"),
            RelationshipType::Triggers => write!(f, "Triggers"),
            RelationshipType::Transforms => write!(f, "Transforms"),
            RelationshipType::Constrains => write!(f, "Constrains"),

            // Event
            RelationshipType::Publishes => write!(f, "Publishes"),
            RelationshipType::Subscribes => write!(f, "Subscribes"),
            RelationshipType::Produces => write!(f, "Produces"),
            RelationshipType::Consumes => write!(f, "Consumes"),

            // Inheritance
            RelationshipType::Implements => write!(f, "Implements"),
            RelationshipType::Extends => write!(f, "Extends"),
            RelationshipType::Specializes => write!(f, "Specializes"),
            ...
            RelationshipType::Parallel => write!(f, "Parallel"),
            RelationshipType::Choice => write!(f, "Choice"),
...
impl fmt::Display for ConceptualSpaceId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.0)
    }
}

/// Unique identifier for a quality dimension
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
...
/// Unique identifier for a convex region in conceptual space
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
...
mod tests {
    ...
    fn test_position3d_creation() {
        let pos = Position3D::new(1.0, 2.0, 3.0).unwrap();
    ...
    #[test]
    fn test_position3d_validation() {
        // Valid positions
        assert!(Position3D::new(1.0, 2.0, 3.0).is_ok());
        assert!(Position3D::new(0.0, 0.0, 0.0).is_ok());
        assert!(Position3D::new(-1.0, -2.0, -3.0).is_ok());

        // Invalid positions
        assert!(Position3D::new(f32::NAN, 0.0, 0.0).is_err());
        assert!(Position3D::new(0.0, f32::INFINITY, 0.0).is_err());
        assert!(Position3D::new(0.0, 0.0, f32::NEG_INFINITY).is_err());
        assert!(Position3D::new(f32::NAN, f32::INFINITY, f32::NEG_INFINITY).is_err());
    }
    ...
    fn test_position3d_vec3_conversion() {
        ...
        let pos2: Position3D = vec.try_into().unwrap();
    ...
    fn test_node_content_creation() {
        ...
        let content = NodeContent {
            ...
            node_type: NodeType::Unidentified,
            properties: properties.clone(),
        };

        assert_eq!(content.label, "Test Node");
        assert_eq!(content.node_type, NodeType::Unidentified);
    ...
    fn test_node_type_variants() {
        // Test equality
        assert_eq!(NodeType::Unidentified, NodeType::Unidentified);
        assert_ne!(NodeType::Unidentified, NodeType::Concept);
    ...
    fn test_serialization_roundtrip() {
        ...
        let pos = Position3D::new(1.5, 2.5, 3.5).unwrap();
    ...
    fn test_position3d_is_finite() {
        ...
        let finite_pos = Position3D::new(1.0, 2.0, 3.0).unwrap();
        assert!(finite_pos.is_finite());

        // Test NaN positions using unchecked constructor
        let nan_x = Position3D::new_unchecked(f32::NAN, 2.0, 3.0);
        assert!(!nan_x.is_finite());

        let nan_y = Position3D::new_unchecked(1.0, f32::NAN, 3.0);
        assert!(!nan_y.is_finite());

        let nan_z = Position3D::new_unchecked(1.0, 2.0, f32::NAN);
        assert!(!nan_z.is_finite());

        // Test infinite positions using unchecked constructor
        let inf_x = Position3D::new_unchecked(f32::INFINITY, 2.0, 3.0);
        assert!(!inf_x.is_finite());

        let neg_inf_y = Position3D::new_unchecked(1.0, f32::NEG_INFINITY, 3.0);
    ...
    fn test_relationship_type_display() {
        ...
        // Test process morphisms
        assert_eq!(RelationshipType::FlowsTo.to_string(), "FlowsTo");
        assert_eq!(RelationshipType::Triggers.to_string(), "Triggers");
        assert_eq!(RelationshipType::Transforms.to_string(), "Transforms");
        assert_eq!(RelationshipType::Constrains.to_string(), "Constrains");

        // Test event morphisms
        assert_eq!(RelationshipType::Publishes.to_string(), "Publishes");
        assert_eq!(RelationshipType::Subscribes.to_string(), "Subscribes");
        assert_eq!(RelationshipType::Produces.to_string(), "Produces");
        assert_eq!(RelationshipType::Consumes.to_string(), "Consumes");

        // Test inheritance morphisms
        assert_eq!(RelationshipType::Implements.to_string(), "Implements");
        assert_eq!(RelationshipType::Extends.to_string(), "Extends");
        assert_eq!(RelationshipType::Specializes.to_string(), "Specializes");

        // Test version control morphisms
        ...
        // Test workflow morphisms
        assert_eq!(RelationshipType::Sequence.to_string(), "Sequence");
        assert_eq!(RelationshipType::Parallel.to_string(), "Parallel");
        assert_eq!(RelationshipType::Choice.to_string(), "Choice");
    ...
    fn test_all_node_types() {
        ...
        for node_type in types {
            // Verify they're all different from custom
        ...
        assert_eq!(NodeType::Commit, NodeType::Commit);
        assert_eq!(NodeType::Branch, NodeType::Branch);
