...
impl SubgraphLayoutCalculator {
    ...
    fn calculate_centroid(&self, positions: &HashMap<NodeId, Position3D>) -> Position3D {
        ...
        let sum = positions.values().fold(
            ...
            |acc, pos| Position3D::new(acc.x + pos.x, acc.y + pos.y, acc.z + pos.z),
        );

        let count = positions.len() as f32;
        Position3D::new(sum.x / count, sum.y / count, sum.z / count)
    }
    ...
    fn calculate_weighted_center(
        ...
        if let Some(g) = graph {
            ...
            let mut weighted_sum = Position3D::new(0.0, 0.0, 0.0);
                    ...
                    for j in (i + 1)..nodes.len() {
                        if let (Some(pos_i), Some(pos_j)) =
                            ...
                            forces.entry(nodes[i])
                                .or_insert(Position3D::new(0.0, 0.0, 0.0))
                                .x -= fx;
                            forces.entry(nodes[i])
                                .or_insert(Position3D::new(0.0, 0.0, 0.0))
                                .y -= fy;
...
impl GridLayout {
    ...
    pub fn calculate(
        ...
        for (i, &node_id) in nodes.iter().enumerate() {
            ...
            positions.insert(node_id, Position3D::new(x, y, center.z));
        }
...
mod tests {
    ...
    fn test_centroid_calculation() {
        ...
        positions.insert(NodeId::new(), Position3D::new(2.0, 2.0, 0.0));
