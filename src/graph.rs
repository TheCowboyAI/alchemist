    ...
    pub fn to_snarl_graph(&self) -> Snarl<GraphNodeData> {
        ...
        for edge in self.edges.values() {
                ...
                (node_id_map.get(&edge.source), node_id_map.get(&edge.target))
            {
                ...
                let out_pin_id = egui_snarl::OutPinId {
                    node: *source_id,
                    output: 0,
                };
        ...
        match event.event_type {
            ...
            GraphEventType::NodeDeleted => {
                if let Some(entity_id) = event.entity_id {
                    ...
                    self.edges
                        .retain(|_, edge| edge.source != entity_id && edge.target != entity_id);
                }
            }
...
impl GraphWorkflow {
    ...
    pub fn create_example_workflow(&mut self) {
        ...
        // Connect nodes
        self.graph
            .add_edge(node1, node2, vec!["flows_to".to_string()]);
        self.graph
            .add_edge(node2, node3, vec!["flows_to".to_string()]);
    }
    ...
    pub fn create_decision_workflow(&mut self) {
        ...
        let process_node = self
            .graph
            .add_node("Process Data", vec!["process".to_string()]);
        let decision_node = self
            .graph
            .add_node("Decision", vec!["decision".to_string()]);
        let end_node = self.graph.add_node("End", vec!["end".to_string()]);

        // Connect nodes with edges
        self.graph
            .add_edge(start_node, process_node, vec!["flows_to".to_string()]);
        self.graph
            .add_edge(process_node, decision_node, vec!["flows_to".to_string()]);
        self.graph
            .add_edge(decision_node, end_node, vec!["true_path".to_string()]);

        // Set positions for better layout
        self.graph
            .node_positions
            .insert(start_node, egui::pos2(200.0, 300.0));
        self.graph
            .node_positions
            .insert(process_node, egui::pos2(450.0, 300.0));
        self.graph
            .node_positions
            .insert(decision_node, egui::pos2(700.0, 300.0));
        self.graph
            .node_positions
            .insert(end_node, egui::pos2(950.0, 300.0));
    }
}
