                ...
                if let Some(cursor_position) = window.cursor_position() {
                    ...
                    if let Some(ray) = ray {
                        ...
                        for (origin, transform) in subgraph_origins.iter() {
                            let distance = ray.intersect_sphere(
                                transform.translation,
                                0.5, // Radius for click detection
                            );
...
pub fn preview_merge(
    ...
    mut preview_query: Query<Entity, With<MergePreview>>,
) {
            ...
            events.send(CommandEvent {
                command: Command::SubgraphOperation(
                    SubgraphOperationCommand::MergeSubgraphs {
                        ...
                        strategy: crate::domain::value_objects::MergeStrategy::UnionAll,
                    }
                ),
            });
        }
                    ...
                    if let Some(cursor_position) = window.cursor_position() {
                        ...
                        if let Some(ray) = ray {
                            ...
                            for (origin, transform) in subgraph_origins.iter() {
                                let distance = ray.intersect_sphere(
                                    transform.translation,
                                    2.0, // Larger radius for subgraph area
                                );
...
pub fn preview_split(
    ...
    mut preview_query: Query<Entity, With<SplitPreview>>,
    mut gizmos: Gizmos,
) {
    // Remove old preview
    for entity in preview_query.iter() {
...
pub fn execute_split(
    ...
    if mouse_button.just_released(MouseButton::Left) && split_state.can_split {
        if let Some(subgraph_id) = split_state.splitting_subgraph {
            // Create split criteria based on the line
            let criteria = if let Some(ref split_line) = split_state.split_line {
                crate::domain::value_objects::SplitCriteria::BySpatialPartition {
                    plane_normal: crate::domain::value_objects::Position3D {
                        x: split_line.normal.x,
                        y: split_line.normal.y,
                        z: split_line.normal.z,
                    },
                    plane_point: crate::domain::value_objects::Position3D {
                        x: split_line.start.x,
                        y: split_line.start.y,
                        z: split_line.start.z,
                    },
                }
            } else {
                crate::domain::value_objects::SplitCriteria::ByNodeCount { target_count: 2 }
            };
