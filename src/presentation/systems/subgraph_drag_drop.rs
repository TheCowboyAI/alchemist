use bevy::prelude::*;
use crate::domain::value_objects::{SubgraphId, NodeId, Position3D};
                ...
                if let Some(cursor_position) = window.cursor_position() {
                    ...
                    if let Some(ray) = ray {
                        ...
                        for (entity, node, transform, draggable, member) in draggables.iter() {
                            ...
                            if let Some(distance) = ray.intersect_sphere(
                        ...
                        if let Some((entity, node, transform, member)) = closest_hit {
                            ...
                            let hit_point = ray.origin + ray.direction * closest_distance;
                            ...
                            state.dragging = Some(DraggingInfo {
                                ...
                                offset,
                                from_subgraph: member.and_then(|m| m.subgraph_ids.first().copied()),
                            });
                        }
                    }
                }
                ...
                if let Ok(window) = windows.get_single() {
                    if let Some(cursor_position) = window.cursor_position() {
                        ...
                        if let Some(ray) = ray {
                            ...
                            let t = (dragging.current_position.y - ray.origin.y) / ray.direction.y;
                            let hit_point = ray.origin + ray.direction * t;

                            // Apply offset
                            let new_position = hit_point - dragging.offset;

                            // Apply constraints
                            if let Ok((mut transform, draggable)) = dragged_nodes.get_mut(dragging.entity) {
                                let constrained_position = apply_drag_constraints(
                                    new_position,
                            ...
                            for (origin, zone_transform, drop_zone) in drop_zones.iter() {
                                let distance = dragging.current_position.distance(zone_transform.translation);
                                ...
                                let is_valid = match &drop_zone.accepts {
                                    ...
                                    DropAcceptance::FromSubgraphs(allowed) => {
                                        dragging.from_subgraph.map_or(false, |id| allowed.contains(&id))
                                    }
                                    DropAcceptance::NotFromSubgraphs(forbidden) => {
                                        dragging.from_subgraph.map_or(true, |id| !forbidden.contains(&id))
                                    }
                                    DropAcceptance::Custom(_) => true, // TODO: Implement custom logic
                                };

                                state.drop_zones.push(DropZoneInfo {
                                ...
                                if distance < radius && is_valid {
                                    state.hover_zone = Some(origin.subgraph_id);
                                }
                            }
                        }
                    }
            ...
            if let Some(target_subgraph) = state.hover_zone {
                ...
                events.send(CommandEvent {
                    command: Command::SubgraphOperation(
                        SubgraphOperationCommand::MoveNodeBetweenSubgraphs {
                            node_id: dragging.node_id,
                            from_subgraph: dragging.from_subgraph,
                ...
                events.send(CommandEvent {
                    command: Command::SubgraphOperation(
                        SubgraphOperationCommand::RemoveNodeFromSubgraph {
                            node_id: dragging.node_id,
