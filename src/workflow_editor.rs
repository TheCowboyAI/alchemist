...
impl WorkflowEditor {
    ...
    pub fn ui(&mut self, ctx: &mut egui::Context) {
        ...
        let mut window = egui::Window::new("Workflow Editor").default_size([800.0, 600.0]);
    ...
    fn show_editor_content(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            if ui.button("Add Node").clicked() {
                let node_id = self
                    .workflow
                    .graph
                    .add_node("New Node", vec!["workflow".to_string()]);
        ...
        if let Some(snarl_graph) = &mut self.workflow.snarl_graph {
            ...
            let _response = snarl_graph.show(&mut viewer, &style, "workflow_editor", ui);
                    ...
                    egui::Frame::popup(ui.style()).show(ui, |ui| {
                        ...
                        if ui.button("Add Input Node").clicked() {
                            let node_id = self
                                .workflow
                                .graph
                                .add_node("42", vec!["input".to_string()]);
                            self.selected_node = Some(node_id);
                        ...
                        if ui.button("Add Math Node").clicked() {
                            let node_id = self
                                .workflow
                                .graph
                                .add_node("a + b", vec!["math".to_string()]);
                            self.selected_node = Some(node_id);
                        ...
                        if ui.button("Add Multiply Node").clicked() {
                            let node_id = self
                                .workflow
                                .graph
                                .add_node("foo * bar", vec!["multiply".to_string()]);
                            self.selected_node = Some(node_id);
                            self.node_sizes.insert(node_id, egui::vec2(200.0, 120.0)); // Wider size
                            self.collapsed_nodes.insert(node_id, false);
                            // Initialize with default values
                        ...
                        if ui.button("Add Division Node").clicked() {
                            let node_id = self
                                .workflow
                                .graph
                                .add_node("x / y", vec!["division".to_string()]);
                            self.selected_node = Some(node_id);
                            self.node_sizes.insert(node_id, egui::vec2(200.0, 120.0)); // Wider size
                            self.collapsed_nodes.insert(node_id, false);
                            // Initialize with default values
