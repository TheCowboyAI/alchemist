...
impl EntityRegistry {
    ...
    pub fn get_component<T: 'static + Clone>(
        &self,
        entity: Uuid,
        component_name: &str,
    ) -> Option<T> {
    ...
    pub fn export_to_graph(&self) -> AlchemistGraph {
        ...
        for (entity_id, components) in &self.registry.entities {
            ...
            if components.contains_key("node") {
                if let Some(node_comp) = self
                    .registry
                    .get_component::<NodeComponent>(*entity_id, "node")
                {
                    let node = GraphNode {
                        id: *entity_id,
                        name: node_comp.name,
            ...
            if components.contains_key("edge") {
                if let Some(edge_comp) = self
                    .registry
                    .get_component::<EdgeComponent>(*entity_id, "edge")
                {
                    let edge = GraphEdge {
                        id: *entity_id,
    ...
    pub fn update_positions(&mut self) {
        ...
        for entity_id in entity_ids {
            if self.registry.has_component(entity_id, "node")
                && self.registry.has_component(entity_id, "position")
            {
                if let Some(mut pos) = self
                    .registry
                    .get_component::<PositionComponent>(entity_id, "position")
                {
    ...
    pub fn process_workflow(&mut self) {
        ...
        for (entity_id, components) in &self.registry.entities {
            if components.contains_key("workflow") && components.contains_key("node") {
                if let Some(workflow) = self
                    .registry
                    .get_component::<WorkflowComponent>(*entity_id, "workflow")
                {
        ...
        for node_id in workflow_nodes {
            if let Some(node) = self
                .registry
                .get_component::<NodeComponent>(node_id, "node")
            {
        ...
        match event.event_type {
            GraphEventType::NodeCreated => {
                if let Some(entity_id) = event.entity_id {
                    ...
                    self.registry
                        .add_component(entity_id, "position", component);
            ...
            GraphEventType::NodeUpdated => {
                if let Some(entity_id) = event.entity_id {
                    if self.registry.has_component(entity_id, "node") {
                        if let Some(mut node_comp) = self
                            .registry
                            .get_component::<NodeComponent>(entity_id, "node")
                        {
                        ...
                        (event.payload.get("source"), event.payload.get("target"))
                    {
                            ...
                            (Uuid::parse_str(source_str), Uuid::parse_str(target_str))
                        {
                            let labels = event
                                ...
                                .get("labels")
                                .map(|s| {
                                    s.split(',').map(|label| label.trim().to_string()).collect()
                                })
                                .unwrap_or_else(Vec::new);
            ...
            GraphEventType::EdgeUpdated => {
                if let Some(entity_id) = event.entity_id {
                    if self.registry.has_component(entity_id, "edge") {
                        if let Some(mut edge_comp) = self
                            .registry
                            .get_component::<EdgeComponent>(entity_id, "edge")
                        {
                            // Update properties from payload
...
impl EcsEditor {
    pub fn create_default_graph(&mut self) {
        ...
        let position_id = self
            .graph
            .add_node("Position", vec!["component".to_string()]);
        let velocity_id = self
            .graph
            .add_node("Velocity", vec!["component".to_string()]);
        ...
        let movement_id = self
            .graph
            .add_node("Movement System", vec!["system".to_string()]);
        let collision_id = self
            .graph
            .add_node("Collision System", vec!["system".to_string()]);

        // Connect entities to components
        self.graph
            .add_edge(player_id, position_id, vec!["has".to_string()]);
        self.graph
            .add_edge(player_id, velocity_id, vec!["has".to_string()]);
        self.graph
            .add_edge(player_id, health_id, vec!["has".to_string()]);

        self.graph
            .add_edge(enemy_id, position_id, vec!["has".to_string()]);
        self.graph
            .add_edge(enemy_id, velocity_id, vec!["has".to_string()]);
        self.graph
            .add_edge(enemy_id, health_id, vec!["has".to_string()]);
        self.graph
            .add_edge(enemy_id, damage_id, vec!["has".to_string()]);

        // Connect systems to components they access
        self.graph
            .add_edge(movement_id, position_id, vec!["accesses".to_string()]);
        self.graph
            .add_edge(movement_id, velocity_id, vec!["accesses".to_string()]);

        self.graph
            .add_edge(collision_id, position_id, vec!["accesses".to_string()]);
        self.graph
            .add_edge(collision_id, health_id, vec!["accesses".to_string()]);
        self.graph
            .add_edge(collision_id, damage_id, vec!["accesses".to_string()]);
    }
    ...
    pub fn connect_entity_to_component(&mut self, entity_id: Uuid, component_id: Uuid) {
        self.graph
            .add_edge(entity_id, component_id, vec!["has".to_string()]);
    }

    pub fn connect_system_to_component(&mut self, system_id: Uuid, component_id: Uuid) {
        self.graph
            .add_edge(system_id, component_id, vec!["accesses".to_string()]);
    }
}
