...
pub fn apply_base16_theme(ctx: &mut egui::Context, theme: &Base16Theme) {
    ...
    style.visuals.window_fill = theme.base00; // Use darker background
    style.visuals.panel_fill = theme.base00; // Use darker background
    style.visuals.faint_bg_color = theme.base01;
    style.visuals.extreme_bg_color = theme.base00;

    // Text colors - much better contrast
    style.visuals.override_text_color = Some(theme.base06); // Brighter text

    // Interactive widgets - improved contrast
    style.visuals.widgets.noninteractive.bg_fill = theme.base01;
    style.visuals.widgets.noninteractive.fg_stroke = egui::Stroke::new(1.0, theme.base05);

    style.visuals.widgets.inactive.bg_fill = theme.base01;
    style.visuals.widgets.inactive.fg_stroke = egui::Stroke::new(1.0, theme.base06);

    style.visuals.widgets.hovered.bg_fill = theme.base02;
    style.visuals.widgets.hovered.fg_stroke = egui::Stroke::new(1.5, theme.base07);

    style.visuals.widgets.active.bg_fill = theme.base0D;
    ...
    style.visuals.selection.stroke = egui::Stroke::new(1.0, theme.base07);
...
pub fn theme_selector_ui(ui: &mut egui::Ui, theme: &mut AlchemistTheme) {
    ui.group(|ui| {
        ...
        let theme_names: Vec<String> = theme
            .available_themes
            .iter()
            .map(|t| t.name.clone())
            .collect();
            ...
            egui::ComboBox::new("theme_selector", "")
                ...
                .show_ui(ui, |ui| {
                    for (idx, name) in theme_names.iter().enumerate() {
                        if ui
                            .selectable_label(theme.selected_theme_index == idx, name)
                            .clicked()
                        {
