...
use crate::graph_patterns::{GraphPattern, generate_pattern, PatternCatalog};
...
impl Default for DddEditor {
    fn default() -> Self {
        ...
        catalog.add_pattern("entity", GraphPattern::Tree { branch_factor: 2, depth: 2 });
        ...
        Self {
            ...
            bounded_contexts: vec!["Core Domain".to_string(), "Supporting".to_string(), "Generic".to_string()],
            aggregates: vec!["User".to_string(), "Order".to_string(), "Product".to_string()],
            entities: vec!["Customer".to_string(), "Order".to_string(), "LineItem".to_string(), "Product".to_string()],
            value_objects: vec!["Address".to_string(), "Money".to_string(), "Email".to_string(), "PhoneNumber".to_string()],
            visible: false,
            window_pos: None,
        }
    }
...
impl DddEditor {
    pub fn create_default_graph(&mut self) {
        let pattern = GraphPattern::Tree { branch_factor: 2, depth: 2 };
    ...
    fn transform_to_ddd_structure(&mut self) {
        ...
        for (_id, node) in self.graph.nodes.iter_mut() {
            ...
            if node.name == "Root" {
                ...
                node.properties.insert("type".to_string(), "Domain".to_string());
            } 
            // Second level nodes are aggregates
                ...
                node.properties.insert("type".to_string(), "Aggregate".to_string());
            }
            // Leaf nodes are entities or value objects
                ...
                if rand::random::<bool>() {
                    ...
                    node.properties.insert("type".to_string(), "Entity".to_string());
                } else {
                    node.name = format!("ValueObject_{}", node.name.replace("Grandchild", ""));
                    node.labels.push("ValueObject".to_string());
                    node.properties.insert("type".to_string(), "ValueObject".to_string());
                }
            }
        }
