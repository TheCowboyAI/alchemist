...
impl GraphEditor {
    ...
    pub fn ui(&mut self, ctx: &mut egui::Context) {
        ...
        let mut window = egui::Window::new("Graph Editor").default_size([800.0, 600.0]);
    ...
    fn show_editor_content(&mut self, ui: &mut egui::Ui) {
        ...
        if let Some(snarl_graph) = &mut self.snarl_graph {
            ...
            let _response = snarl_graph.show(&mut viewer, &style, "graph_editor", ui);
                ...
                if let Some(pos) = ui.input(|i| i.pointer.hover_pos()) {
                    ...
                    for (node_id, node_data) in snarl_graph.nodes_ids_data() {
                        ...
                        if let Some(node_info) = snarl_graph.get_node_info(node_id) {
                            ...
                            let node_size = self
                                .node_sizes
                                .entry(node_data.value.uuid)
                                .or_insert(default_size)
                                .clone();
            ...
            if self.show_context_menu {
                ...
                area.show(ui.ctx(), |ui| {
                    ...
                    egui::Frame::popup(ui.style()).show(ui, |ui| {
                        if ui.button("Add Node").clicked() {
                            let node_id =
                                self.graph.add_node("New Node", vec!["basic".to_string()]);
                            self.selected_node = Some(node_id);
                        ...
                        if ui.button("Add Process Node").clicked() {
                            let node_id =
                                self.graph.add_node("Process", vec!["process".to_string()]);
                            self.selected_node = Some(node_id);
                            self.node_sizes.insert(node_id, egui::vec2(200.0, 120.0));
                            self.collapsed_nodes.insert(node_id, false);
                            self.sync_to_snarl();
                        ...
                        if ui.button("Add Connection Node").clicked() {
                            let node_id = self
                                .graph
                                .add_node("Connection", vec!["connection".to_string()]);
                            self.selected_node = Some(node_id);
                            self.node_sizes.insert(node_id, egui::vec2(200.0, 120.0));
                            self.collapsed_nodes.insert(node_id, false);
                            self.sync_to_snarl();
        ...
        if let Some(selected_node) = self.selected_node {
            ...
            ui.vertical(|ui| {
                ...
                if node_labels.contains(&"data".to_string()) {
                    ...
                    let node_contents =
                        self.node_contents.entry(selected_node).or_insert_with(|| {
                            let mut map = HashMap::new();
                            map.insert("value".to_string(), "".to_string());
                            map
                        });

                    let mut value = node_contents
                        .get("value")
                        .unwrap_or(&"".to_string())
                        .clone();
                    ...
                    let mut description = node_contents
                        .get("description")
                        .unwrap_or(&"".to_string())
                        .clone();
    ...
    fn create_example_graph(&mut self) {
        ...
        let node2 = self
            .graph
            .add_node("Process A", vec!["process".to_string()]);
        let node3 = self
            .graph
            .add_node("Process B", vec!["process".to_string()]);
        ...
        self.graph
            .add_edge(node3, node4, vec!["produces".to_string()]);
        ...
        node2_data.insert(
            ...
            "Transforms input data".to_string(),
        );
