...
pub enum GraphPattern {
    ...
    FiniteAutomaton { states: usize, alphabet_size: usize, is_deterministic: bool },
    DirectedAcyclicGraph { levels: usize, nodes_per_level: usize },
    Bipartite { left_nodes: usize, right_nodes: usize, edge_density: f32 },
}
...
impl GraphPattern {
    ...
    pub fn name(&self) -> &'static str {
        match self {
            ...
            GraphPattern::FiniteAutomaton { is_deterministic, .. } => {
                if *is_deterministic { "Deterministic Finite Automaton (DFA)" } 
                else { "Non-deterministic Finite Automaton (NFA)" }
            },
    ...
    pub fn description(&self) -> &'static str {
        match self {
            GraphPattern::Tree { .. } => "A hierarchical structure with a root node and child nodes",
            GraphPattern::Star { .. } => "A central node connected to multiple other nodes",
            GraphPattern::Cycle { .. } => "Nodes connected in a ring where each node has exactly two neighbors",
            ...
            GraphPattern::MooreMachine => "State machine where outputs depend only on the current state",
            GraphPattern::MealyMachine => "State machine where outputs depend on both current state and inputs",
            GraphPattern::FiniteAutomaton { is_deterministic, .. } => {
                if *is_deterministic {
            ...
            GraphPattern::DirectedAcyclicGraph { .. } => "A directed graph with no cycles, often used to represent dependencies",
            GraphPattern::Bipartite { .. } => "A graph whose vertices can be divided into two disjoint sets",
        }
    }
}
...
impl PatternCatalog {
    ...
    pub fn new() -> Self {
        ...
        catalog.add_pattern("binary_tree", GraphPattern::Tree { branch_factor: 2, depth: 3 });
        ...
        catalog.add_pattern("simple_dfa", GraphPattern::FiniteAutomaton { 
            states: 4, 
            alphabet_size: 2, 
            is_deterministic: true 
        });
        catalog.add_pattern("simple_nfa", GraphPattern::FiniteAutomaton { 
            states: 4, 
            alphabet_size: 2, 
            is_deterministic: false 
        });
        catalog.add_pattern("small_dag", GraphPattern::DirectedAcyclicGraph { levels: 3, nodes_per_level: 2 });
        catalog.add_pattern("basic_bipartite", GraphPattern::Bipartite { left_nodes: 3, right_nodes: 3, edge_density: 0.7 });
        catalog.add_pattern("small_grid", GraphPattern::Grid { width: 3, height: 3 });
...
pub fn generate_pattern(pattern: GraphPattern) -> AlchemistGraph {
    ...
    match pattern {
        ...
        GraphPattern::RegularPolygon { sides } => {
            generate_regular_polygon(&mut graph, sides)
        },
        GraphPattern::MooreMachine => {
            generate_moore_machine(&mut graph)
        },
        GraphPattern::MealyMachine => {
            generate_mealy_machine(&mut graph)
        },
        GraphPattern::FiniteAutomaton { states, alphabet_size, is_deterministic } => {
        ...
        GraphPattern::Bipartite { left_nodes, right_nodes, edge_density } => {
...
fn generate_regular_polygon(graph: &mut AlchemistGraph, sides: usize) {
    ...
    for i in 0..sides {
        ...
        graph.nodes.get_mut(&node_id).unwrap().properties.insert("x_pos".to_string(), x.to_string());
        graph.nodes.get_mut(&node_id).unwrap().properties.insert("y_pos".to_string(), y.to_string());
        graph.nodes.get_mut(&node_id).unwrap().properties.insert("fixed_position".to_string(), "true".to_string());
    ...
    for i in 0..sides {
        ...
        graph.add_edge(node_ids[i], node_ids[next_i], vec!["polygon_edge".to_string()]);
    }
}

fn generate_moore_machine(graph: &mut AlchemistGraph) {
    // Create states
    let disconnected = graph.add_node("Disconnected", vec!["state".to_string(), "initial".to_string(), "output=inactive".to_string()]);
    let connected = graph.add_node("Connected", vec!["state".to_string(), "output=active".to_string()]);
    let error = graph.add_node("Error", vec!["state".to_string(), "terminal".to_string(), "output=error".to_string()]);
    
    // Add transitions (without outputs)
    graph.add_edge(disconnected, connected, vec!["transition".to_string(), "input=connect".to_string()]);
    graph.add_edge(connected, disconnected, vec!["transition".to_string(), "input=disconnect".to_string()]);
    graph.add_edge(connected, error, vec!["transition".to_string(), "input=error".to_string()]);
    graph.add_edge(error, disconnected, vec!["transition".to_string(), "input=reset".to_string()]);
}
...
fn generate_mealy_machine(graph: &mut AlchemistGraph) {
    ...
    if let Some(node) = graph.nodes.get_mut(&waiting) {
        node.properties.insert("fixed_position".to_string(), "true".to_string());
        node.properties.insert("x_pos".to_string(), "-200.0".to_string());
        node.properties.insert("y_pos".to_string(), "0.0".to_string());
        node.properties.insert("z_pos".to_string(), "0.0".to_string());
    }
    
    if let Some(node) = graph.nodes.get_mut(&processing) {
        node.properties.insert("fixed_position".to_string(), "true".to_string());
        node.properties.insert("x_pos".to_string(), "0.0".to_string());
        node.properties.insert("y_pos".to_string(), "0.0".to_string());
        node.properties.insert("z_pos".to_string(), "0.0".to_string());
    }
    ...
    if let Some(node) = graph.nodes.get_mut(&error) {
        node.properties.insert("fixed_position".to_string(), "true".to_string());
        node.properties.insert("x_pos".to_string(), "0.0".to_string());
        node.properties.insert("y_pos".to_string(), "200.0".to_string());
        node.properties.insert("z_pos".to_string(), "0.0".to_string());
    }
    
    // Add transitions with outputs on the transitions (Mealy machine)
    graph.add_edge(waiting, processing, vec!["transition".to_string(), "input=start".to_string(), "output=initializing".to_string()]);
    graph.add_edge(processing, completed, vec!["transition".to_string(), "input=complete".to_string(), "output=success".to_string()]);
    graph.add_edge(processing, error, vec!["transition".to_string(), "input=fail".to_string(), "output=error".to_string()]);
    graph.add_edge(completed, waiting, vec!["transition".to_string(), "input=reset".to_string(), "output=ready".to_string()]);
    graph.add_edge(error, waiting, vec!["transition".to_string(), "input=reset".to_string(), "output=retry".to_string()]);
}

fn generate_finite_automaton(graph: &mut AlchemistGraph, states: usize, alphabet_size: usize, is_deterministic: bool) {
...
fn generate_bipartite(graph: &mut AlchemistGraph, left_nodes: usize, right_nodes: usize, edge_density: f32) {
