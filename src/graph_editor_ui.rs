use crate::graph_editor_3d::{
    CreatePatternEvent, GraphEditor3D, UiInteractionState, UpdateGraph3DEvent,
};
    ...
    egui::Window::new("3D Graph Editor")
        ...
        .show(ctx, |ui| {
            ...
            if theme.use_custom_theme {
                ...
                if ui
                    .checkbox(&mut theme.use_dark_theme, "Dark Mode")
                    .clicked()
                {
                    // Automatically switch between Dracula (dark) and Default Light
                    if theme.use_dark_theme {
                        // Find Dracula theme
                        if let Some(position) = theme
                            .available_themes
                            .iter()
                            .position(|t| t.name == "Dracula")
                        {
                        ...
                        if let Some(position) = theme
                            .available_themes
                            .iter()
                            .position(|t| t.name == "Default Light")
                        {
                            theme.selected_theme_index = position;
                            theme.current_theme = theme.available_themes[position].clone();
                        }
